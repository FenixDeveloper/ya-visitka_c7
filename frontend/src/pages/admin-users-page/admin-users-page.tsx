...
export const AdminUsersPage: FC = () => {
  ...
  const handleUpdate = (student: TStudent) => {
    ...
    if (studentToUpdate) {
      ...
      if (student.fromFile) {
        ...
        api.putUsers({ email: student.email, cohort: student.cohort }, student.id)
          .then(student => {
  ...
  const updateUser = async (student: TStudent, isExisting: boolean, failedUploadsArr: TStudent[]) => {
    if (isExisting) {
      await api.putUsers({ email: student.email, cohort: student.cohort }, student.id)
        .catch((err) => {
      ...
      await api.postUsers({ email: student.email, cohort: student.cohort })
        .catch((err) => {
          console.log(err.message);
          failedUploadsArr.push(student)
        });
  ...
  const handleSave = async () => {
    ...
    const uploadArray = virtualStudents.map((virtualStudent) => {
      ...
      return updateUser(virtualStudent, isExisting, failedToUpload);
    });
